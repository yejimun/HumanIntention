...
class IntentionSampler:
    ...
    def visualize_intent(self, traj_end_pos=None):
        ...
        # ax.set(xlim=(165, 380), ylim=(80, 260))
        ...
        plt.ylim([80, 240])
        # ax.axis('scaled')
        plt.savefig('ex_visualize_2intention.png')
        return fig, ax
        
    def belong2intent(self, point):
        vec_bf2point = point - self.intent_bottom_left_coordinates
        # !! 1.5 --> ?
        bool_flag = ((vec_bf2point[:, 0] > 0) * (vec_bf2point[:, 0] < self.intent_wid)) * \
            ((vec_bf2point[:, 1] > 0) * (vec_bf2point[:, 1] < self.intent_wid))
    ...
    def generate_bottom_left_positions(self):
        # !! we have different xy range for our data
        x_bottom_left_list = [np.array([330, 200]), np.array([330, 90])]
...
if __name__ == "__main__":
    ...
    intentions = np.arange(i_sampler.intent_num) #np.arange(3,19)
